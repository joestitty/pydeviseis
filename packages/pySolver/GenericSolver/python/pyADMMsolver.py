...
class BoundedADMMsolver(ADMMsolver):
    """
    ADMM solver that enforces velocity bounds at every step
    """
    def __init__(self, proxf, proxg, outer, rho, rho_adjust_ratio, rho_decr, rho_incr, 
                 save_second, save_dual, min_vel, max_vel, wave_eq_solver):
        super().__init__(proxf, proxg, outer, rho, rho_adjust_ratio, rho_decr, 
                         rho_incr, save_second, save_dual)
        self.min_vel = min_vel
        self.max_vel = max_vel
        self.wave_eq_solver = wave_eq_solver
        print(f"BoundedADMMsolver initialized with bounds: {self.min_vel} - {self.max_vel}")
    
    def enforce_bounds(self, vector):
        """Enforce velocity bounds on a vector"""
        # Get array
        arr = vector.getNdArray()
        
        # Truncate padding to get actual model
        unpadded = self.wave_eq_solver._truncate_model(arr)
        
        # Apply bounds
        np.clip(unpadded, self.min_vel, self.max_vel, out=unpadded)
    ...
    def run(self, verbose=False):
        """Run the ADMM algorithm with bounds enforcement"""
        z_prev = self.z.clone()
        
        for it in range(self.outer):
            if verbose:
                print(f"\nADMM Iteration {it+1}/{self.outer}")
            
            # Compute z_k - u_k with bounds enforcement
            temp = self.z.clone()
            temp.scaleAdd(self.u, 1.0, -1.0)
            
            # Enforce bounds on input to prox_f
            self.enforce_bounds(temp)
            
            # x_k+1 = proxf(z_k - u_k)
            self.proxf.prox(temp, self.x, 1/self.rho)
            
            # Enforce bounds on x after prox_f
            self.enforce_bounds(self.x)
            
            # Compute x_k+1 + u_k with bounds enforcement  
            temp = self.x.clone()
            temp.scaleAdd(self.u, 1.0, 1.0)
            
            # Enforce bounds on input to prox_g
            self.enforce_bounds(temp)
            
            # z_k+1 = proxg(x_k+1 + u_k)
            self.proxg.prox(temp, self.z, 1/self.rho)
            
            # Enforce bounds on z after prox_g
            self.enforce_bounds(self.z)
            
            # r_k+1 = x_k+1 - z_k+1 (primal residual)
            self.r = self.x.clone()
            self.r.scaleAdd(self.z, 1.0, -1.0)
            
            # u_k+1 = u_k + r_k+1 (dual update)
            self.u.scaleAdd(self.r, 1.0, 1.0)
            
            # Update rho if needed
            if self.rho_adjust_ratio > 0:
                r_norm = self.r.norm()
                s_norm = z_prev.clone().scaleAdd(self.z, -1.0).norm() * self.rho
                
                if r_norm > self.rho_adjust_ratio * s_norm:
                    self.rho *= self.rho_incr
                elif s_norm > self.rho_adjust_ratio * r_norm:
                    self.rho *= self.rho_decr
            
            # Save current z for next iteration
            z_prev.copy(self.z)
